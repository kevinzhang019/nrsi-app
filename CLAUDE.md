...
- **Status:** Vercel frontend at https://nrsi-app.vercel.app; Railway supervisor cron `0 12 * * *`; 251/251 unit tests passing; build green
...
- **Graceful non-Final exit:** every watcher exit path that isn't the Final branch (`max-loops`, `max-runtime`, `abort` SIGTERM, `error` uncaught throw) calls `services/lib/finalize-game.ts:performGracefulExit` with the last successfully-published state. The helper does one final feed fetch: if MLB has flipped to Final it runs the full Final path (persist to Supabase + clear watcher state); otherwise it republishes `{ ...lastState, status: "Final" }` so the dashboard isn't stuck. Best-effort — never throws. The main loop body in `services/run-watcher.ts` is wrapped in a try/catch that routes any uncaught step error into `gracefulExit("error")` instead of bubbling to the supervisor. See BUGS.md bug #11.
