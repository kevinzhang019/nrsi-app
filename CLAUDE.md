...
- **Status:** Vercel frontend at https://nrsi-app.vercel.app; Railway supervisor cron `0 12 * * *`; 257/257 unit tests passing; build green
...
| 12 | `/history` randomly missing per-inning predictions — `gracefulExit("abandoned")` discarded `capturedInnings` instead of writing them to Supabase, captures expired with the watcher's 24h Redis TTL | `services/lib/finalize-game.ts` (abandoned branch persists too) |
...
- **Hard runtime caps:** `MAX_LOOPS = 5000` (≈7h at the 5s active cadence) and `MAX_RUNTIME_MS = 6h`. The wall-clock cap is the real ceiling; MAX_LOOPS is a defense against tight-loop bugs. Historically MAX_LOOPS was 1500 (≈2h) and watchers hit it during the late innings of normal games, exiting silently and leaving stuck-Live snapshots — see BUGS.md bug #11.
- **Graceful non-Final exit:** every watcher exit path that isn't the Final branch (`max-loops`, `max-runtime`, `abort` SIGTERM, `error` uncaught throw) calls `services/lib/finalize-game.ts:performGracefulExit` with the last successfully-published state AND `capturedInnings`. The helper does one final feed fetch: if MLB has flipped to Final it runs the full Final path (persist to Supabase + clear watcher state); otherwise it republishes `{ ...lastState, status: "Final" }` so the dashboard isn't stuck AND persists the captured per-inning predictions (+ partial play rows from the last feed) so they don't expire silently with the 24h Redis TTL. Skipped only when both `capturedInnings` and `playRows` are empty. Best-effort — never throws. The main loop body in `services/run-watcher.ts` is wrapped in a try/catch that routes any uncaught step error into `gracefulExit("error")` instead of bubbling to the supervisor. See BUGS.md bug #11 (stuck-Live cleanup) and bug #12 (per-inning persistence).
...
- **`performGracefulExit` "abandoned" branch persists captured innings, not just publishes synthetic Final.** The persist call lives in `services/lib/finalize-game.ts:192-206`, runs BEFORE `publishUpdate`, is wrapped in its own try/catch, and is skipped only when both `capturedInnings` and `playRows` are empty. Reverting to the publish-only behavior reintroduces bug #12 — captured per-inning predictions evaporate with the watcher's 24h Redis TTL because no automatic re-watch picks up an abandoned game (supervisor cron only schedules today's games, snapshot already says synthetic-Final so prune sweeps it tomorrow). The skip-when-empty guard prevents an empty `games` row for a watcher that died before any half-boundary. `playRows` defaults to `[]` when the final feed fetch failed OR `buildPlayRows` throws — `inning_predictions` are independently valuable, don't gate the persist on plays
- **Stale-live detector runs in the supervisor's idle loop:** `services/supervisor.ts` calls `detectAndCleanStaleLive` once per `IDLE_CHECK_INTERVAL_MS` (60s) iteration. Errors are caught and warn-logged so a transient Redis hiccup doesn't break the supervisor. Folding the detector before the break check would skip it on past-deadline supervisors (off-season days) — that's fine because there are no Live games to clean then anyway, but moving it AFTER the sleep delays first-pass cleanup by 60s
