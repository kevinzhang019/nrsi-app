...
Each batter row in `components/lineup-column.tsx` shows exactly four fields, left → right: **bats** (handedness) · **F. Lastname** · **xOBP** · **xSLG**. The marker dot, batting-order spot number, and position abbreviation were intentionally removed — the at-bat signal is conveyed solely by the row-level background highlight (`bg-[var(--color-accent-soft)]/60`), and the on-deck (next half-inning leadoff) signal by a 1px green outline (`border-[var(--color-accent)]`). Both states use the same accent color so the card has a single alert palette; filled = batting now, outlined = batting next half. To keep the bordered and non-bordered rows the same height, every row gets `border border-transparent` as a base and the highlighted row only swaps the border color. The lineup column header used to render `AT BAT` / `ON DECK` pills next to the team label — those were removed so the row treatment is the single focus signal. The `highlightKind` prop on `<LineupColumn>` is `"current" | "next" | null`; `<GameCard>` derives it from `game.battingTeam` and pairs it with `currentBatterId` (for the batting team) or `nextHalfLeadoffId` (for the waiting team).
