...
The **bats** field is `HandCode | null` and gets hydrated from `/people/{id}` (cached 30d via `loadHand`) by `workflows/steps/enrich-lineup-hands.ts` — the live-feed boxscore omits `batSide` for most players, so reading it raw silently produces 18 right-handers per game (see BUGS.md bug #7). The render falls back to `"—"` if enrichment somehow misses, keeping the column width stable.
