...
The dashboard groups today's games into four sections in fixed order: **Highlighted → Active → Upcoming → Finished**. Each is a separate `<AnimatePresence mode="popLayout">` parent, and each card is a `<motion.div layout layoutId={`card-${gamePk}`}>` so a card animates smoothly when `decisionMomentFor(game, predictMode)` flips (Active ↔ Highlighted) or when status changes (Pre → Live → Final).

The Highlighted section's membership rule (`lib/state/decision-moment.ts:decisionMomentFor`): a Live game qualifies when its half just ended (`outs >= 3`) OR a fresh half is sitting with no PA-produced outs yet (`outs === 0 && bases === (inning >= 10 ? 2 : 0)`). Runs scored on a leadoff PA (e.g. a leadoff HR) do not disqualify — only outs and bases do. In full mode, the transition narrows to `half === "Bottom"` (full inning ended) and fresh-half narrows to `half === "Top"` (new inning starting). The `bases === 2` allowance in extras matches the Manfred-runner-on-2B state the Markov layer treats as the clean half-boundary; any other base configuration disqualifies. The predicate is decoupled from the server-side `isDecisionMoment` / `isDecisionMomentFullInning` flags — those still ride along on GameState but the UI no longer reads them, so dashboard policy can be tuned without touching the watcher.
...
1. `services/supervisor.ts` schedules each watcher at `gameDate − 6h` (`PRE_GAME_LEAD_MS = 6h`) — day games spawn at supervisor wake, night games wait in `setTimeout` (zero resource cost) until their window opens. Each spawned watcher polls MLB at 30-min cadence while `status === "Pre"`. The 6h window is the conservative tail of when MLB actually publishes lineups (typically T-1h to T-3h) and locks probable pitchers (usually by T-6h); going wider would just burn lock-refresh + feed-poll cost for no signal.
...
4. The dashboard's `useGameStream` SSE merges the new state into its `Map<gamePk, GameState>`, the Upcoming-section card re-renders, and the `<ProbabilityPill>` switches from the empty `nrXi / —` placeholder to `P(nr1i) / <odds>`. The `<LineupColumn>` rows populate, the pitcher row shows the probable starter, the park / weather chips fill in. The status header in `<InningState>` shows the **viewer-local start time** (e.g. "7:10 PM") instead of the MLB `detailedState` string ("Scheduled" / "Warmup" / "Pre-Game"); the time is more useful at-a-glance and naturally reflects each viewer's timezone. The `<LineScore>` strip stays hidden in Pre — the watcher's published state carries `linescore: null` while `status === "Pre"` (see "Don't change without thinking" below).
...
- `linescore: status === "Pre" ? null : extractLinescore(tick.feed)` at `services/run-watcher.ts:696`. `extractLinescore` is non-null even when innings are empty (`{ innings: [], totals: zeros }`), and `<GameCard>` gates its `<LineScore>` strip on plain truthiness. Without the status gate, every pre-game-compute tick flips the snapshot from the seed stub's `linescore: null` to the truthy-empty shape, surfacing a `1 2 3 4 5 6 7 8 9 \| R H E` row of zeros under a "SCHEDULED" header on the dashboard. See BUGS.md bug #14.
- The `mounted` flag in `components/inning-state.tsx`'s Pre branch — required so SSR and client first render produce identical HTML. The server lives in UTC; the client lives in the viewer's TZ; formatting the same UTC ISO produces different strings on each side, which is a hydration mismatch + visible flicker. Server + client first render both fall through to `detailed || "Pre-game"`; the local time string only appears after `useEffect` runs. Replacing with `suppressHydrationWarning` silences the warning but keeps the flicker. See BUGS.md bug #14.
...
Stroke transitions `var(--color-muted) → var(--color-accent)` over 240ms when `highlighted` flips, so the outline lights up **in lockstep** with the existing highlight ring + `flash-fresh` keyframe. `highlighted` is fed by `decisionMomentFor(game, predictMode)` (the same predicate that decides the Highlighted dashboard section), so the park outline, card ring, and section assignment all flip together. One unified accent (green — `--color-accent` is now `#22c55e`) alert state across the whole card; no competing visual cues.
...
**Mobile overflow handling.** The page's `<main>` uses `mx-auto max-w-[1400px] px-4 py-6 sm:px-6 sm:py-8` (`app/history/[pk]/page.tsx`) — `px-4` on phones reclaims 16px of viewport on each side before the `<GameCard>` is rendered. The card itself stays `overflow-hidden` (load-bearing for the ring + animations), so every child that can be wider than the viewport must self-scroll: `<LineScore>` (`components/line-score.tsx` — `overflow-x-auto`), `<LineupColumn>` (`components/lineup-column.tsx` — `overflow-x-auto` + `min-w-max`), `<PitcherRow>` (`flex flex-wrap`). Don't tighten the page padding further than `px-4` on phones — `<LineScore>` cells become un-tappable below ~14px each.
...
**Mobile overflow handling.** The three grid containers (half-inning view, full-inning Top, full-inning Bottom) use `grid gap-4 p-4 sm:gap-6 sm:p-6 md:grid-cols-2` — tighter padding + gap on phones so the inner usable width on a 360px device stays around 296px instead of ~264px. All three inner sections must self-scroll horizontally because rows are nominally wider than that:
- `HitterTable` — `<div className="overflow-x-auto">` + `<table className="w-full min-w-max">`.
- `PitcherTable` — same pattern.
- `PlayLog` — `<div className="overflow-x-auto">` + `<ol className="min-w-max">`, with `whitespace-nowrap` on each `<li>` so long batter / pitcher names + event text (`"Field Error by Shortstop"`, `"Strikeout — Looking, Wild Pitch"`) push the row to scroll instead of wrapping mid-line. Don't drop `min-w-max` from the `<ol>` or the row collapses back into its parent's content-box and the scroll wrapper does nothing.
...
- The `PlayLog` `overflow-x-auto` wrapper + `min-w-max` on the `<ol>` + `whitespace-nowrap` on every `<li>` are a single unit. Removing any one of the three lets long event/player text push the panel off-screen on phones — the surrounding card uses `overflow-hidden`, so what overflows is clipped, not scrolled.
