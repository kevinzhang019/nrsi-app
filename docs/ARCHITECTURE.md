...
4. Loop up to `MAX_LOOPS = 1500` AND `Date.now() - startedAt < 6h`:
   ...
   - Build `structuralKey` (half | inning | lineup | defense | opposing pitcher | atBat batter) and `playStateKey` (outs | bases | atBatIndex). Set `shouldReloadStructure` and `shouldRecomputePlay` triggers.
   - **Phase 1 (structural reload):** parallel fetch `loadLineupSplitsStep`, `loadParkFactorStep`, `loadWeatherStep`, `loadDefenseStep`. Compute display lineup stats for both teams. Pre-compute opposite-half clean-state P(no run) for full-inning composition. Persist to hoisted `lastNrXi`, `lastEnv`, `lastPitcher*`, `lastLineupStats`, etc.
