...
```
                  ...
                  │  - per game: setTimeout(gameDate-6h, runWatcher)
                  │      day games spawn at supervisor wake;
                  │      night games idle in setTimeout (~0 cost)
                  │      until their slot — covers the realistic
                  │      lineup-posting window (T-3h typical, T-6h
                  │      conservative)
   ...
   │     adaptive sleep 5s/15s/1800s/300s            │             │
   │       (live active PA / live break / Pre / Delayed)             │
...
2. **6h pre-game window with 30-min cadence.** `PRE_GAME_LEAD_MS = 6h`. Day games (start ≤ 18:00 UTC) hit `Math.max(now, gameDate - 6h)` resolving to supervisor wake and start watching immediately; night games idle in `setTimeout` (zero Redis / MLB / lock cost) until their slot arrives later in the day. 6h is the conservative tail of when MLB actually publishes lineups (typically T-1h to T-3h) and locks probable pitchers (usually by T-6h). The watcher computes first-inning predictions during pre-game whenever lineups + probable pitchers are available; the same structural-key change detector that drives mid-game recomputes catches any pre-game lineup/pitcher/defense change on the next 30-min tick. Cost: ~12 ticks per game over the 6h pre-game window, each tick is one `fetchLiveDiff` + ~25 Redis hits — negligible since splits/park/weather are all heavily cached. The 6h ceiling is the explicit resource-efficiency knob: going wider (e.g., 24h) burns Redis lock-refresh ops (10s cadence × extra hours × every game) and MLB polls for no signal during the long stretch when MLB hasn't published anything yet. Persistence to `inning_predictions` stays gated on `status === "Live"` (see Watcher Lifecycle step 4) so pre-game previews surface on the dashboard but never write to Supabase. See `services/supervisor.ts:PRE_GAME_LEAD_MS` and `services/run-watcher.ts:isPregameReady`.
...
Daily entry point invoked by `bin/supervisor.ts`. Fetches today's schedule, calls `seedSnapshotStep` (hsetnx Pre stubs), runs `pruneStaleSnapshots({ todayET: date })` to wipe field-keys whose row's own `officialDate < todayET`, then enqueues per-game watcher tasks with `setTimeout(gameDate - 6h, runWatcher)`. Day games spawn at supervisor wake (the `max(now, ...)` falls through to `now`); night games idle in setTimeout — zero resource cost while waiting. Once spawned, watchers run pre-game ticks at 30-min cadence until first pitch. Tracks `pending: Set<gamePk>` and exits cleanly when the set drains AND we're past `defaultIdleDeadline()` (next-day 06:00 UTC). On SIGTERM, drains watchers up to a 30s budget, then exits.
...
4. Loop up to `MAX_LOOPS = 5000` AND `Date.now() - startedAt < 20h`. Loop body wrapped in try/catch so any uncaught throw routes into `gracefulExit("error")` instead of bubbling:
   - `fetchLiveDiffStep` with last seen `metaData.timeStamp` for tiny diff payloads.
   - Build `structuralKey` (half | inning | lineup | defense | opposing pitcher | atBat batter | bottom-9-skipped flag) and `playStateKey` (outs | bases | atBatIndex). Set `shouldReloadStructure` and `shouldRecomputePlay` triggers. The compute predicate is `canCompute = isLive || isPregameReady`, where `isPregameReady` is `status === "Pre" && upcoming !== null && upcoming.pitcherId !== null` (= both lineups posted with 9-deep `battingOrder` + probable pitchers known). Pre-game ticks run the same Phase 1 + Phase 2 pipeline as live ticks; the structural-key diff captures any lineup/pitcher/defense change on the next 30-min tick and re-fires Phase 1.
   ...
   - Adaptive sleep: 5s during active PAs, 15s otherwise (Live), **1800s (30 min) Pre**, 300s Delayed/Suspended. The Live branches come from `services/steps/fetch-live-diff.ts:chooseRecommendedWaitSeconds`; the Pre / Delayed / Suspended branches are hard-coded in `services/run-watcher.ts`'s sleep switch. The long pre-game cadence is fine because the structural-key change detector catches any lineup/pitcher/defense diff on the next tick — a starter scratched at T-45min surfaces a refreshed prediction within ≤ 30 min, and a starter scratched in the last 30 minutes still gets caught on the very next live tick after status flips (the live recompute uses current inputs and the capture fires from that).

5. Non-Final exits (`max-loops`, `max-runtime`, `abort` from SIGTERM, `error` from any uncaught throw) all call `services/lib/finalize-game.ts:performGracefulExit`. When `lastPublishedState.status === "Live"`, the helper publishes `{ ...lastPublishedState, status: "Final" }` so the dashboard moves the game out of Active, then clears `nrxi:watcher-state:{gamePk}`. When `lastPublishedState.status === "Pre"` (a watcher dying mid-pre-game — most realistically `abort` from a Railway redeploy or `error` from an uncaught feed-parse, since `max-runtime = 20h` no longer fires inside the 6h pre-game window), the synthetic Final publish is **skipped** — leaving the Upcoming card in place is correct because the next supervisor cron will spawn a fresh watcher; the watcher-state is still cleared so the replacement rebuilds from scratch. **No DB writes here** in either branch — captured per-inning predictions are already in Supabase from the per-boundary fire-and-forget upserts, and the supervisor's `sweepFinalize` will land `games`/`plays`/`actual_runs` once MLB flips the game to Final. Best-effort, never throws. Hoisted `lastPublishedState: GameState | null` (set after every successful `publishUpdateStep`) is the source. See [BUGS.md bug #11](BUGS.md#bug-11-snapshots-stuck-live-at-varying-innings-after-a-watcher-exits-without-cleanup) and [bug #12](BUGS.md#bug-12-history-missing-per-inning-predictions-when-the-watcher-exits-via-graceful-exit-abandoned).
...
Per-game card. Header is a 3-column grid `[logo | team name | runs]` × 2 rows (away on top, home below) with `<InningState>` floated right. Below the header: `<LineScore>` (with logo-prefixed row labels), pitcher line, two `<LineupColumn>`s, footer with `<ProbabilityPill>`. The dashboard highlight predicate (`lib/state/decision-moment.ts:decisionMomentFor` — fires on `outs >= 3` or `outs === 0 && bases === (inning >= 10 ? 2 : 0)`, mode-gated for half/full) adds `ring-2 ring-[var(--color-accent)]/60` plus a brief flash and lights up the `<ParkOutline>` in lockstep. See [UI.md](UI.md) → "Dashboard sectioning" for the predicate semantics and "Score header" for the logo / color invariants.
