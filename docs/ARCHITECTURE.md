...
```
                  ...
                  │  - per game: setTimeout(gameDate-24h, runWatcher)
                  │      resolves to Date.now() for any today-game →
                  │      all watchers start immediately at supervisor wake
...
2. **Long pre-game window with 30-min cadence.** `PRE_GAME_LEAD_MS = 24h`, so `Math.max(now, gameDate - LEAD)` resolves to `now` for every game on today's schedule — every watcher spawns immediately at supervisor wake and runs through to first pitch + game-end. Pre-game tick cadence is 30 min (hard-coded in `services/run-watcher.ts`'s sleep switch). The watcher computes first-inning predictions during pre-game whenever lineups + probable pitchers are available; the same structural-key change detector that drives mid-game recomputes catches any pre-game lineup/pitcher/defense change on the next 30-min tick. Cost: ~24 ticks per game over a 12h pre-game window (vs. ~3 ticks under the legacy 90s lead), each tick is one `fetchLiveDiff` + ~25 Redis hits — negligible since splits/park/weather are all heavily cached. Persistence to `inning_predictions` stays gated on `status === "Live"` (see Watcher Lifecycle step 4) so pre-game previews surface on the dashboard but never write to Supabase. See `services/supervisor.ts:PRE_GAME_LEAD_MS` and `services/run-watcher.ts:isPregameReady`.
...
Daily entry point invoked by `bin/supervisor.ts`. Fetches today's schedule, calls `seedSnapshotStep` (hsetnx Pre stubs), runs `pruneStaleSnapshots({ todayET: date })` to wipe field-keys whose row's own `officialDate < todayET`, then enqueues per-game watcher tasks with `setTimeout(gameDate - 24h, runWatcher)` — which resolves to `Date.now()` for every today-game, so all watchers spawn immediately at supervisor wake and run pre-game ticks at 30-min cadence until first pitch. Tracks `pending: Set<gamePk>` and exits cleanly when the set drains AND we're past `defaultIdleDeadline()` (next-day 06:00 UTC). On SIGTERM, drains watchers up to a 30s budget, then exits.
...
- **MAX_LOOPS = 5000 + 20h wall-clock cap.** The wall-clock cap is sized to comfortably outlast the supervisor (which idle-exits at 06:00 UTC next day, ≈18h after wake) — watchers spawn at supervisor wake under `PRE_GAME_LEAD_MS = 24h` and must stay alive through the full pre-game + game window. The pre-pre-game-compute cap was 6h, which fired at 18:00 UTC under the new lead and flipped every scheduled game to a synthetic Final before games even started; the bump to 20h fixes that. MAX_LOOPS (≈7h at 5s active cadence) is a defense against tight-loop bugs; with pre-game cadence at 1800s, it accrues only ~24 iterations across the pre-game window. Non-Final exits run `gracefulExit`, which now skips the synthetic-Final publish when `lastPublishedState.status === "Pre"` so a watcher dying mid-pre-game leaves the Upcoming card intact instead of moving it to Finished. See BUGS.md bug #11.
